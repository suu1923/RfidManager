...
class Validate
{
     ...
     * 检测数据是否重复
     */
    protected function isRepeat($value,$rule='',$data='',$key,$field){
        $modelClass = str_replace("\\validate\\", "\\model\\", $this->validate);
        try{
            $table = new $modelClass();
            $result = $table->where($key,$value)->find();
            return !$result ? true : $field.$value."已经存在";
